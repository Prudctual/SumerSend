...
export function startQueueWorker() {
  ...
  try {
    realtimeChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('[Worker Realtime] New message queued. Triggering processor...');
          processMessageQueue().catch(err => console.error('[Worker] Error in realtime message processor:', err));
        }
      )
      .on(
        'postgres_changes',
        ...
        (payload) => {
          // If the update set status back to pending, process it
          if (payload.new && payload.new.status === 'pending') {
            console.log('[Worker Realtime] Message updated to pending. Triggering processor...');
            processMessageQueue().catch(err => console.error('[Worker] Error in realtime message processor:', err));
          }
        }
      )
      .on(
        ...
        (payload) => {
          console.log('[Worker Realtime] New webhook queued. Triggering processor...');
          processWebhookQueue().catch(err => console.error('[Worker] Error in realtime webhook processor:', err));
        }
      )
      .on(
        'postgres_changes',
        ...
        (payload) => {
          if (payload.new && payload.new.status === 'pending') {
            console.log('[Worker Realtime] Webhook updated to pending. Triggering processor...');
            processWebhookQueue().catch(err => console.error('[Worker] Error in realtime webhook processor:', err));
          }
        }
      )
  ...
  // Safety fallback polling every 15 seconds
  ...
  processMessageQueue().catch(err => console.error(err));
  processWebhookQueue().catch(err => console.error(err));
}
...
export async function processWebhookQueue() {
  ...
  try {
    ...
    while (hasMore) {
      ...
      console.log(`[Worker] Processing batch of ${webhooks.length} webhooks...`);
